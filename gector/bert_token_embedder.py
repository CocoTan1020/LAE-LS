...
class PretrainedBertModel:
    ...
    @classmethod
    def load(cls, model_name: str, cache_model: bool = True, pre_pretrained_model_path=None) -> PreTrainedModel:
        if model_name in cls._cache:
            return PretrainedBertModel._cache[model_name]
        if pre_pretrained_model_path is None:
            model = AutoModel.from_pretrained(model_name)
        else:
            model = AutoModel.from_pretrained(pre_pretrained_model_path)
        if cache_model:
...
class BertEmbedder(TokenEmbedder):
    ...
    def forward(
        ...
        offsets : ``torch.LongTensor``, optional
            ...
            will be returned.
        for_sequence : For simplabel prediction, ues the '[CLS]' token embedding as the
        ...
        if need_attention:
            ...
            all_encoder_layers_all = self.bert_model(
                input_ids=util.combine_initial_dims(input_ids),
                attention_mask=util.combine_initial_dims(input_mask),
                output_attentions=True
            )
            attention = all_encoder_layers_all['attentions'][-1]
            return attention

        # 只用于对抗学习中，使用generator的logits输出与discriminator的bertembedding相乘所得到的logits_embedding[batch_size, sequence_length, 768]，
        # 将该logits_embedding作为bertmodel的输入，
        # 输出logits_embed[batch_size, 768]
        if logits_embedding is not None:
            cls_embedding = self.bert_model(inputs_embeds=logits_embedding, attention_mask=attention_mask)[1]
        ...
        if return_cls == True:
            ...
            cls_embedding = all_encoder_layers_all[1]
        ...
        all_encoder_layers_all = all_encoder_layers_all[0:2]
        # print('all_encoder_layers_all', all_encoder_layers_all, len(all_encoder_layers_all))

        all_encoder_layers = all_encoder_layers_all[0]
        # print(all_encoder_layers.shape)
        cls_embedding = all_encoder_layers_all[1]
        # print(cls_embedding.shape)
        # print('all_encoder_layers.shape', all_encoder_layers.shape)  # torch.Size([32, 47, 768])
        # print('cls_embedding.shape', cls_embedding.shape)  # torch.Size([32, 768])
        # print('all_encoder_layers[0].shape', all_encoder_layers[0].shape) torch.Size([47, 768])
        ...
        # print('all_encoder_layers.shape', all_encoder_layers.shape) torch.Size([1, 32, 47, 768])
        ...
        # print('mix.shape', mix.shape) # torch.Size([32, 47, 768])
        ...
        if self.for_sequence:
            ...
            if offsets is None:
                ...
                # print('offsets.shape', offsets.shape) # torch.Size([32, 40])
                offsets2d = util.combine_initial_dims(offsets)
                # print('offsets2d.shape', offsets2d.shape) # torch.Size([32, 40])
                # now offsets is (batch_size * d1 * ... * dn, orig_sequence_length)
                range_vector = util.get_range_vector(
                    offsets2d.size(0), device=util.get_device_of(mix)
                ).unsqueeze(1)
                # print('range_vector.shape', range_vector.shape) # torch.Size([32, 1])
                # selected embeddings is also (batch_size * d1 * ... * dn, orig_sequence_length)
                selected_embeddings = mix[range_vector, offsets2d]
                # print('selected_embeddings.shape', selected_embeddings.shape) # torch.Size([32, 40, 768])

                # print('util.uncombine_initial_dims(selected_embeddings, offsets.size())', util.uncombine_initial_dims(selected_embeddings, offsets.size()).shape) # torch.Size([32, 40, 768])
...
class PretrainedBertEmbedder(BertEmbedder):
    ...
    def __init__(
        ...
        for_sequence: bool = False,
        for_logits: bool = False,
    ) -> None:
        model = PretrainedBertModel.load(pretrained_model, pre_pretrained_model_path=pre_pretrained_model_path)
