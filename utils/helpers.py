...
def wordlistlist2sentence(word_list_list):
    sentences = []
...
def write_json_data(source_sentences, predict_sentences, tags, filename):
    source_sentences = wordlistlist2sentence(source_sentences)
    predict_sentences = wordlistlist2sentence(predict_sentences)
    tags = wordlistlist2sentence(tags)
    # print('source_sentences', source_sentences)
    # print('predict_sentences', predict_sentences)
    d_list = []
    for s, t, p in zip(source_sentences, tags, predict_sentences):
        d = {
            "source_sentence": s,
            "predict_tags": t,
            "predict_sentence": p
...
def write_json_for(dict, filename):
    d_list = []
    for s, t, p in zip(dict['original_sentence_all'], dict['post_tag_batch_all'], dict['tags_all']):
        d = {
            "original_sentence": " ".join(s),
            "post_tag_batch": " ".join(t),
            "tags": " ".join(p)
        }
        d_list.append(d)
    # print(d_list)
...
def metadata2senteces(metadata):
    sentences = []
    for mm in metadata:
        words = mm['words']
        sentence = [i for i in words[1:]]
        sentences.append(sentence)
    return sentences
