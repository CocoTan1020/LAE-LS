...
from gector.datareader import Seq2SimpLabelDatasetReader
from gector.seq2simplabel_model import Seq2SimpLabel
from gector.trainer import Trainer_Discriminator
...
log = Logger('log_discriminator.txt')
sys.stdout = log
...
def get_token_embedders(model_name, tune_bert=False, special_tokens_fix=0, pre_pretrained_model_path=None):
...
def get_model(model_name, vocab, tune_bert=False,
    ...
    token_embs = get_token_embedders(model_name, tune_bert=tune_bert, special_tokens_fix=special_tokens_fix, pre_pretrained_model_path=pre_pretrained_model_path)
...
def main(args):
    ...
    if args.vocab_path:
        ...
        print('Vocabulary should be provided.')
    ...
    # model.load_state_dict(torch.load('finished_trained_models/pretrained_discriminator/model_state_epoch_19.th', map_location=torch.device('cpu')))
    ...
    trainer = Trainer_Discriminator(model=model,
                      optimizer=optimizer,
                      scheduler=scheduler,
                      iterator=iterator,
